...
export default function DesigningServices() {
  return (
    <section id="designing" className="py-24 bg-gradient-to-br from-white via-teal-50 to-purple-50 dark:from-gray-950 dark:via-purple-900/20 dark:to-gray-950">
      <div className="max-w-7xl mx-auto px-4">
        <h2 className="text-4xl md:text-5xl font-extrabold text-center text-teal-600 dark:text-teal-400 mb-14 tracking-tight drop-shadow-lg">Designing Services</h2>
        {categories.map((cat, idx) => (
          <div key={cat.title} className="mb-16">
            <h3 className="text-2xl font-bold text-teal-500 dark:text-purple-300 mb-8 text-center uppercase tracking-wide">{cat.title}</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-10">
              {cat.services.map((service, i) => (
                <motion.div
                  ...
                  className="bg-white/90 dark:bg-gray-800/80 p-6 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 flex flex-col items-center hover:scale-105 hover:shadow-2xl hover:border-teal-400 dark:hover:border-purple-400 transition-all duration-300 backdrop-blur-md group"
                >
                  <div className="relative">
                    <img src={service.img} alt={service.title} className="h-20 w-20 rounded-full object-cover shadow-lg mb-4 border-4 border-teal-100 dark:border-purple-200 group-hover:border-teal-300 dark:group-hover:border-purple-300 transition-colors duration-200" />
                  ...
                  <h4 className="text-base font-bold text-gray-800 dark:text-gray-100 text-center mb-1 tracking-tight group-hover:text-teal-600 dark:group-hover:text-purple-400 transition-colors duration-200">{service.title}</h4>
                </motion.div>
              ))}
            </div>
          </div>
